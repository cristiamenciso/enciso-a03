...
// class to initialize list
    ...
    public void printTreeList(List<TreeMap<String, Employee>> tList) {
        System.out.println("Name" + "                 |  " + "Position            |" + "   Separation Date            |");
        System.out.println("---------------------|----------------------|----------------------|");
        for(TreeMap<String, Employee> m : tList) {
            Collection<Employee> emp = m.values();
            for (Employee e : emp) {
                System.out.format("%-10s%-15s%-20s%-15s", e.getFirstName(), e.getLastName(), e.getRole(), e.getSeparationDate());
                System.out.println();
    ...
    public List<TreeMap<String, Employee>> sortList(List<HashMap<String, Employee>> list) {
        List<HashMap<String, Employee>> newList = new ArrayList<>();
        List<TreeMap<String, Employee>> treeList = new ArrayList<>();
        Object[] keys = new Object[list.size()];
        String[] strKeys = new String[list.size()];
